...
bool Url::parse(Url* base) {
	// If the URL we start with is of zero length, it's just the same URL as whatever passed in
	if(strlen(m_url) == 0) {
		int length = strlen(base->get_url());
		m_url = (char*)malloc(length + 1);
		strcpy(m_url, base->get_url());
		return(_split());
	}

	// Get the scheme out of the URL
	char* scheme = (char*)malloc(4 + 1);
	strncpy(scheme, m_url, 4);
	scheme[4] = '\0';
	...
	// If this is just an anchor tag, ignore it
	if(strncmp(m_url, "#", 1) == 0) {
		int length = strlen(base->get_url());
                m_url = (char*)malloc(length + 1);
                strcpy(m_url, base->get_url());
	...
	// If we're here, our new path begins from the last forward slash in the base path
	char* base_path = base->get_path();
	char* copy_end = strrchr(base_path, '/');

	int copy_length = (copy_end == NULL) ? 1 : (copy_end - base_path) + 1;
	int url_length = strlen(m_url);
	char* temp = (char*)malloc(copy_length + url_length + 1);
	if(copy_length > 0) {
		strncpy(temp, base_path, copy_length);
	}
	else {
		char* copy = "/";
		strncpy(temp, copy, 1);
	...
	unsigned int final_path_length = strlen(m_url) - offset + 1;
	...
	strcpy(m_url + base_copy_length, final_path + offset + 1);
